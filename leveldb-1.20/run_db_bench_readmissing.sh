...
do
  ...
  do
    ...
    $DB_BENCH --db=$DB_PATH --benchmarks=fillseq --threads=1 --num=1000000 --value_size=100

    echo $c

    # 关键改为 readmissing
    taskset -c 0-$(($c-1)) $lock $DB_BENCH --pin_threads=1 --benchmarks=readmissing --use_existing_db=1 --db=$DB_PATH --threads=$c --time_ms=30000 | tee $result_dir/readmissing.$c
