...
do
	...
	taskset -c 0-$(($c-1)) $lock $DB_BENCH --pin_threads=1 --benchmarks=readwhilewriting --use_existing_db=1 --db=$DB_PATH --threads=$c --time_ms=30000 | tee $result_dir/readwrite.$c
